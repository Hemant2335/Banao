...
const Banner = ({joingroup , setjoingroup} : BannerProps) => {
  return (
    <div className="w-full md:h-[60vh] relative mb-[2vh]">
      ...
      {!joingroup && ( <button onClick={()=>setjoingroup(true)} className="md:hidden absolute top-2 right-2 border-2 text-white text-sm p-3 rounded-lg flex gap-2 items-center">
        <FiUserPlus /> Join Group
      </button>)}
       {joingroup && ( <button onClick={()=>setjoingroup(false)} className="md:hidden absolute top-2 right-2 border-2 text-white text-sm p-3 rounded-lg flex gap-2 items-center">Leave Group</button>)}
